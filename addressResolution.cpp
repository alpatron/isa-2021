...
/**
 * @brief Given a hostname understood by the 'getaddrinfo' function, resolves it to an IPv4 or IPv6 address and stores it in the Address wrapper structure. The address object must be freed with the 'freeAddress' function after use. This function is merely a wraper for the 'getaddrinfo' function.
 * 
 * @exception std::runtime_error Thrown if address resolution fails or if the address is resolved to a non-IP address. In this case, the Address structure may be modified but isn't neccessary to be freed.
 * 
...
void resolveNameToAddress(const char* host,Address* address){
    ...
    address->addressLenght = result->ai_addrlen;
